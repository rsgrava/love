...
local function parseSpeed(speed)
    ...
    elseif speed == "slow" then
        ...
        assert(tonumber(speed),
               "setSpeed() - Expected number, got " .. tostring(speed))
        return speed
    end
end
...
function Typer:update(dt)
    ...
    if not self.paused then
        ...
        while not self.paused and not self.complete and self.timer <= 0 do
            ...
            self.paused = utf8.sub(self.strip, self.position + 1,
                                   self.position + 2) == "--"
        end
    end
...
function Talkies.say(title, messages, config)
    ...
    for i = 1, #messages do
        msgFifo:push(Typer.new(messages[i],
                               config.textSpeed or Talkies.textSpeed))
    end
    ...
    local newDialog = {
        ...
        -- theme
        indicatorCharacter = config.indicatorCharacter or
        ...
        optionSwitchSound = config.optionSwitchSound or
            Talkies.optionSwitchSound,
        inlineOptions = config.inlineOptions or Talkies.inlineOptions,
        font = font,
        fontHeight = font:getHeight(" "),
        typedNotTalked = config.typedNotTalked == nil and Talkies.typedNotTalked or
            config.typedNotTalked,
        pitchValues = config.pitchValues or Talkies.pitchValues,

        optionIndex = 1,

        showOptions = function(dialog)
            return dialog.messages:len() == 1 and type(dialog.options) ==
                       "table"
        end,
        isShown = function(dialog)
            return Talkies.dialogs:peek() == dialog
        end
    }

    newDialog.messageBackgroundColor = config.messageBackgroundColor or
                                           Talkies.messageBackgroundColor
    newDialog.titleBackgroundColor = config.titleBackgroundColor or
                                         Talkies.titleBackgroundColor or
                                         newDialog.messageBackgroundColor

    newDialog.messageColor = config.messageColor or Talkies.messageColor
    newDialog.titleColor = config.titleColor or Talkies.titleColor or
                               newDialog.messageColor

    newDialog.messageBorderColor = config.messageBorderColor or
                                       Talkies.messageBorderColor or
                                       newDialog.messageBackgroundColor
    newDialog.titleBorderColor = config.titleBorderColor or
                                     Talkies.titleBorderColor or
                                     newDialog.messageBorderColor

    Talkies.dialogs:push(newDialog)
    if Talkies.dialogs:len() == 1 then Talkies.dialogs:peek():onstart() end
...
function Talkies.update(dt)
    ...
    if currentMessage:update(dt) then
        ...
        elseif not currentDialog.talkSound:isPlaying() then
            local pitch = currentDialog.pitchValues[math.random(
                              #currentDialog.pitchValues)]
...
function Talkies.isOpen() return Talkies.dialogs:peek() ~= nil end
...
function Talkies.draw()
    ...
    local function getDimensions()
        ...
        if canvas then return canvas:getDimensions() end
    ...
    local boxH = Talkies.height or (windowHeight / 3) -
                     (2 * currentDialog.padding)
    local boxX = currentDialog.padding
    local boxY = windowHeight - (boxH + currentDialog.padding)

    -- image
    local imgX, imgY, imgW, imgScale = boxX + currentDialog.padding,
                                       boxY + currentDialog.padding, 0, 0
    if currentDialog.image ~= nil then
        imgScale = (boxH - (currentDialog.padding * 2)) /
                       currentDialog.image:getHeight()
    ...
    if currentDialog.title ~= "" then
        local titleBoxW = currentDialog.font:getWidth(currentDialog.title) +
                              (2 * currentDialog.padding)
        ...
        if currentDialog.thickness > 0 then
            ...
            love.graphics.rectangle("line", boxX, titleBoxY, titleBoxW,
                                    titleBoxH, currentDialog.rounding,
    ...
    if currentDialog.image ~= nil then
        ...
        love.graphics.draw(currentDialog.image, imgX, imgY, 0, imgScale,
                           imgScale)
        love.graphics.pop()
    end
    ...
    while tempPosition < currentMessage.position and
        utf8.len(currentMessage.strip) > 0 do
        local displayLine = modmsg[lineNum]

        local positionAdd = math.min(currentMessage.position - tempPosition + 1,
                                     utf8.len(displayLine))
    ...
    if currentDialog:showOptions() and currentMessage.complete then
        if currentDialog.inlineOptions then
            ...
            local optionLeftPad = currentDialog.font:getWidth(
                                      currentDialog.optionCharacter .. " ")
            for k, option in pairs(currentDialog.options) do
                love.graphics.print(option[1], optionLeftPad + textX +
                                        currentDialog.padding, optionsY +
                                        ((k - 1) * currentDialog.fontHeight))
            end
            love.graphics.print(currentDialog.optionCharacter .. " ",
                                textX + currentDialog.padding, optionsY +
            ...
            local optionText = ""
            for k, option in pairs(currentDialog.options) do
                local newText = (currentDialog.optionIndex == k and
                                    currentDialog.optionCharacter or " ") .. " " ..
                                    option[1]
                optionWidth = math.max(optionWidth,
                                       currentDialog.font:getWidth(newText))
                optionText = optionText .. newText .. "\n"
            end

            local optionsH = (currentDialog.font:getHeight() *
            ...
            love.graphics.setColor(currentDialog.messageBackgroundColor)
            love.graphics.rectangle("fill", optionsX - currentDialog.padding,
                                    optionsY - currentDialog.padding,
                                    optionWidth + currentDialog.padding * 2,
                                    optionsH + currentDialog.padding * 2,
                                    currentDialog.rounding,
                                    currentDialog.rounding)
            ...
            if currentDialog.thickness > 0 then
                ...
                love.graphics.rectangle("line",
                                        optionsX - currentDialog.padding,
                                        optionsY - currentDialog.padding,
                                        optionWidth + currentDialog.padding * 2,
                                        optionsH + currentDialog.padding * 2,
                                        currentDialog.rounding,
    ...
    if Talkies.showIndicator then
        love.graphics.print(currentDialog.indicatorCharacter,
                            boxX + boxW - (2.5 * currentDialog.padding),
...
function Talkies.clearMessages() Talkies.dialogs = Fifo.new() end
